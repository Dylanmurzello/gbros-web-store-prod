    ...
    createPayment: async (ctx, order, amount, args, metadata): Promise<CreatePaymentResult> => {
        ...
        try {
            ...
            if (!sourceId) {
                ...
                return {
                    amount: order.totalWithTax, // Return full order amount
            ...
            // FIX: 2025-10-01 - Include sourceId hash in idempotency key for smart retry handling 🔄
            // Same card retry = same key = Square deduplicates (no double charge)
            // Different card = different key = Square allows retry (customer can fix payment)
            // This solves both: prevents duplicate charges AND allows payment method changes
            const tokenHash = sourceId.substring(0, 12); // Use first 12 chars of payment token as identifier
            const idempotencyKey = `vendure-${order.code}-${tokenHash}`;
            ...
            const response = await withTimeout(
                client.payments.create({
                    ...
                    amountMoney: {
                        amount: BigInt(order.totalWithTax), // Full order total INCLUDING tax
            ...
            return {
                amount: order.totalWithTax, // Return full amount we charged (including tax)
